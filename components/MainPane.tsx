...
export const MainPane: React.FC<MainPaneProps> = ({ onPromptSelect, onSendMessage, isLoading, customPrompts, onSavePrompt }) => {
  ...
  const displayedPrompts = activeTab === 'Created by me' 
    ? customPrompts 
    : (activeTab === 'Recommended' ? RECOMMENDED_PROMPTS : []);
        ...
        <div className="mt-8 max-w-5xl mx-auto">
          <div className="flex justify-between items-center mb-6">
              <div className="border-b border-gray-200 dark:border-gray-700 w-full flex justify-between">
                  ...
                  <button onClick={() => setCreatePromptModalOpen(true)} className="mb-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors text-sm font-semibold shadow-sm">
                      Create prompt
                  </button>
              </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-12">
            {displayedPrompts.length === 0 ? (
              ...
              displayedPrompts.map((prompt) => (
                <PromptCard key={prompt.id} prompt={prompt} onClick={() => onPromptSelect(prompt.description)} />
              ))
            )}
          </div>
        </div>
